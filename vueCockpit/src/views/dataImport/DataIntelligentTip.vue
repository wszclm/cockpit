            ...
            <div class='marketing-content'>
                <el-row :gutter='10'>
                    <el-col :span='24' align='right'>
                        <el-button type='success' icon='el-icon-download'> <a style="color:#fff" href="../static/file/智能预警.xls" download="智能预警.xls">智能预警模板下载</a></el-button>
                        <el-button type='primary' icon='el-icon-delete-solid' @click='batchRemove'>删除</el-button>
                          </el-col>
                          <el-col>
                           <el-upload
                            ref="upload"
                            action='/apis/intelligentTips/batchImport' 
                ...
                <el-row :gutter='10' style='margin-top: 1%;'>
                    <el-col :span='24' >
                        <el-table ref="table" :data='tableData' :border=true :stripe=true v-loading='loading' @selection-change="changeFun" element-loading-text='拼命加载中'>
                            ...
                            <el-table-column prop='sdanger' align='center' label='安全隐患'></el-table-column>
    ...
    methods: {
        ...
        batchRemove(){
            this.$confirm('此操作将永久删除选中的企业租赁数据, 是否继续?', '提示', {
                ...
                this.deleteRequest('/intelligentTips/deleteById',{ids:ids}).then(resp => {
        ...
        getTableData() {
            ...
            this.getRequest('/intelligentTips/queryAll').then(resp => {
                this.loading = false;
                if (resp) {
