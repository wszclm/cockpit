                ...
                <el-row :gutter='10' style='margin-top: 1%;'>
                    <el-col :span='24' >
                        <el-table ref="table" :data='tableData' :border=true :stripe=true v-loading='loading' @selection-change="changeFun" element-loading-text='拼命加载中'>
                            ...
                            <el-table-column prop='red' align='center' label='红码数量' ></el-table-column>
                            <el-table-column prop='yellow' align='center' label='黄码数量'></el-table-column>
                            <el-table-column prop='orange' align='center' label='橙码数量'></el-table-column>
                            <el-table-column prop='green' align='center' label='绿码数量'></el-table-column>
    ...
    methods: {
        ...
        batchRemove(){
            this.$confirm('此操作将永久删除选中的企业租赁数据, 是否继续?', '提示', {
                ...
                this.deleteRequest('/safeCode/deleteById',{ids:ids}).then(resp => {
        ...
        getTableData() {
            ...
            this.getRequest('/safeCode/queryAll').then(resp => {
                this.loading = false;
                if (resp) {
