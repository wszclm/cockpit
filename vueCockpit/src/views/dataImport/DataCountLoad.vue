<template>
    <el-dialog  title="统计数据导入" top='1%' :visible.sync="isShow"  size="large" class='min-body queryOffer edit-dialog' lock-scroll :modal='true'>
                    <el-form :model="form">
                        <el-row class="margin-bottom-10">
                           ...
                           <!-- <el-col :span="24">
                            ...
                            <el-col :span="24">
                                ...
                                <span><a href="../static/file/统计数据导入模板.xlsx" download="统计数据导入模板.xlsx">统计数据导入模板（Excel）</a>请严格遵循模板格式，否则会导入失败</span>
                            </el-form-item>
                            </el-col>
                            <el-col :span="24">
                                  <el-form-item label="" :label-width="formLabelWidth">
...
export default {
    ...
    methods: {
            ...
            uploadOk(val){
                ...
                this.postMultRequest(this.$CST.LOAD_COUNT_IMPORT, fd).then(resp => {
                this.loading = false;
                if (resp) {
        ...
        urlHandle:function(){
            // 附件上传
            var ret = this.$CST.API_ENTERPRISE_COUNT_FILE.replace('{year}', this.year).replace('{month}', this.month);
            return ret;
        },
...
/*        beforeUpload(file){
    ...
    computed: {
        // this.year = this.$CST.y
        // calAction: function() {
        //      // 附件上传
        //     var ret = this.$CST.API_COUNT_UPLOAD_FILE.replace('{year}', this.year).replace('{month}', this.month);
        //     return ret;
        // },
    },
    watch: {}
