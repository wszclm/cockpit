<template>
    <el-dialog title="税务数据导入" top='1%' :visible.sync="isShow"  size="large" class='min-body queryOffer edit-dialog' lock-scroll :modal='true'>
                    <el-form :model="form">
    ...
    methods: {
         submitUpload() {
            ...
            uploadOk(val){
                ...
                this.postMultRequest(this.$CST.LOAD_TAX_IMPORT, fd).then(resp => {
                this.loading = false;
                if (resp) {
                    if(resp.meta.statusCd == '200'){
                       this.$alert("添加成功："+resp.meta.message,'提示');
                       this.getTableData();
                    }
                }else{
