<template>
    <div>
                ...
                <el-row :gutter='10' style='margin-top: 1%;'>
                    <el-col :span='24' >
                        <el-table :data='tableData' :border=true :stripe=true v-loading='loading' @selection-change="changeFun" element-loading-text='拼命加载中'>
                            ...
                            <el-table-column prop='industryPro' align='center' label='工业产值（万元）'></el-table-column>
                            <el-table-column prop='industryPut' align='center' label='工业增加值（万元）'></el-table-column>
                            <el-table-column prop='industryRatio' align='center' label='R&D经费占主营业务收入比重（%）'></el-table-column>
                            <el-table-column prop='industryAvgPerson' align='center' label='年平均职工人数（人*年）'></el-table-column>
                            <el-table-column prop='energyConsumption' align='center' label='综合能耗（吨标煤）'></el-table-column>
        ...
        <DataCountLoad ref="dataCountLoad" :getTableData="getTableData"></DataCountLoad>
    </div>
</template>
...
<script>
import DataCountLoad from './DataCountLoad'
...
},
{
 ...
 months: [
    ...
    methods: {
        handleAddLabel() {
            this.$refs.dataCountLoad.isShow = true;
        },
        ...
        getTableData() {
            ...
            this.getRequest(this.$CST.LOAD_COUNT_DATA, params).then(resp => {
                this.loading = false;
                if (resp) {
                    ...
                    this.alert('统计数据查询失败：' + reason,'error')
                }
            }).catch((reason) => {
                this.alert('统计数据查询失败：' + reason,'error')
            }).finally(() => {
        ...
        batchRemove(){
            this.$confirm('此操作将永久删除选中的企业统计数据, 是否继续?', '提示', {
                ...
                this.deleteRequest(this.$CST.LOAD_COUNT_DEL,{ids:ids}).then(resp => {
                    if (resp) {
