        ...
        methods: {
            deleteHr(hr) {
                this.$confirm('此操作将永久删除【'+hr.name+'】, 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/apis/system/user/"+hr.id).then(resp=>{
                        if (resp) {
                            this.$message.success(resp.meta.message);
                            this.initUsers();
                        }
                    })
                }).catch(() => {
            ...
            hidePop(hr) {
                ...
                if (flag) {
                    let url = '/apis/system/user/role?hrid=' + hr.id;
            ...
            enabledChange(hr) {
                delete hr.roles;
                this.putRequest("/apis/system/user/", hr).then(resp => {
                    if (resp) {
                        debugger;
                         if(resp.meta.statusCd==-1){
                                 this.$message.error(resp.meta.message);
            ...
            initAllRoles() {
                this.getRequest("/apis/system/user/roles").then(resp => {
                    if (resp) {
                        if(resp.meta.statusCd==-1){
                                 this.$message.error(resp.meta.message);
                                 return;
                        }
                        this.allroles = resp.info;
                    }
                })
            },
            initUsers() {
                this.getRequest("/apis/system/user/?keywords="+this.keywords).then(resp => {
                    if (resp) {
                        this.hrs = resp;
                    }
                })
